...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  private async resolveChatId(chatId: string): Promise<string> {
    if (chatId.endsWith('@c.us')) {
      ...
      try {
        const resolvedId = await this.client!.getNumberId(cleanNumber);
        if (resolvedId) {
          this.logger.debug(`Resolved JID ${chatId} to ${resolvedId._serialized}`);
          return resolvedId._serialized;
        }
      } catch (err) {
        this.logger.warn(`Failed to resolve JID for ${chatId}, falling back to original`, String(err));
      }
    }
    return chatId;
  }
  ...
  async forwardMessage(fromChatId: string, toChatId: string, messageId: string): Promise<MessageResult> {
    this.ensureReady();
    const resolvedToChatId = await this.resolveChatId(toChatId);
    ...
    await msgToForward.forward(resolvedToChatId);
